...
/// Путь к иконке — извлекаем из встроенных байт во временный файл при первом вызове
fn app_icon() -> String {
    use std::sync::OnceLock;
    static ICON_PATH: OnceLock<String> = OnceLock::new();
    ICON_PATH.get_or_init(|| {
        let tmp = std::env::temp_dir().join("transmission-remote-slint-icon.png");
        if std::fs::write(&tmp, include_bytes!("../ui/app-icon.png")).is_ok() {
